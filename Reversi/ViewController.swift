...
extension ViewController {
    ...
    func placeDisk(_ disk: Disk, atX x: Int, y: Int, animated isAnimated: Bool, completion: ((Bool) -> Void)? = nil) throws {
        try boardView.placeDisk(disk, atX: x, y: y, animated: isAnimated) { [weak self] isFinished in
            try? self?.saveGame()
...
extension ViewController {
    ...
    @IBAction func changePlayerControlSegment(_ sender: UISegmentedControl) {
        ...
        if !boardView.isAnimating, side == turn, case .computer = Player(rawValue: sender.selectedSegmentIndex)! {
            playTurnOfComputer()
        }
    }
}
